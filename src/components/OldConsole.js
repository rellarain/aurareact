...
export default function OldConsole({oldConsoleTitle}) {
    // OldConsole Variables
    // const oldConsoleTitle = "account";

    // account dashboard social scholar admin helpDesk
    // assessmentA assessmentG assessmentL assessmentS assessmentX 
    const oldConsoleState = "active";

    const oldConsoleClass = "oldConsole oldConsole" + oldConsoleTitle;
    const today = new Date();
    const time = "8:00";
    const achievementScore = 100;

    const [oldConsoleSidebarClosed, setOldConsoleSidebarClosed] = useState(true);
    const [oldConsoleHelpClosed, setOldConsoleHelpClosed] = useState(true);


    // OldConsole Functions
    function formatDate(date) {
        return new Intl.DateTimeFormat('en-us', {weekday: 'long', month: 'short', day: '2-digit', year:'numeric'}).format(date);
    }
    function handleOldConsoleSidebarClosed() {
        setOldConsoleSidebarClosed(!oldConsoleSidebarClosed);
        // alert("it worked!");
    }
    function handleOldConsoleHelpClosed() {
        setOldConsoleHelpClosed(!oldConsoleHelpClosed);
        // alert("it worked!");
    }
    
    
    return(
    <div className={"oldConsole " + oldConsoleTitle + "OldConsole"}>
        <main className={oldConsoleSidebarClosed ? "oldConsoleSidebarClosed" : "oldConsoleSidebarOpen"}>
            <header>
                <section>{oldConsoleTitle}</section>
                <section>  <span>{formatDate(today)}</span> <span>{time}</span> </section>
                <section> <span style={{width: achievementScore + "%"}}></span> </section>
                <section> <button onClick={handleOldConsoleSidebarClosed}></button> </section>
                <section>
                    <button>Bloop</button>
                    <button>Poop</button>
                ...
                <section><button>II</button></section>
            </header>
            <div className={oldConsoleHelpClosed ? "oldConsoleHelpClosed" : "oldConsoleHelpOpen"}> 
                <section className='oldConsoleSidebar'>
                    <section className='oldConsoleSidebarTabs'>
                        <button></button>
                        <button></button>
                        <button></button>
                        <button></button>
                    ...
                    <section className='oldConsoleSidebarTitle'>
                        Sidebar Title 
                    </section>
                    <section className='oldConsoleSidebarBody'>
                        <section className='oldConsoleSidebarContent'>
                            ...
                            <b>HELP DESK SIDEBAR</b>: for admin;
                                <br/> <b>Styling</b>: the base elements of the oldConsole use personal alert and accent, sidebar and desk use user styling of active chat, 
                        ...
                        <section className='oldConsoleSidebarHelp'>
                            <section>
                                <button onClick={handleOldConsoleHelpClosed}>?</button>

                            </section>
                        </section>
                    </section>
                </section>
                <section className='oldConsoleDesk'>
                    ...
                    <b>ACCOUNT</b>: tools for any user with an active account with Aura Academy (Base = $2.00)
                        <br/> <b>Styling</b>: theme and theme upsat
                        <br/> <b>Progress Bar</b>: 
                        <br/> <b>OldConsole Button</b>: opens sidebar, refresh page; open requests = medium accent, no open requests = medium theme;
                        <br/> <b>OldConsole Tabs</b>: 
                    ...
                    <b>DASHBOARD</b>: available to all users, includes Breaktime as a sidebar
                        ...
                        <br/> <b>OldConsole Button</b>: opens sidebar, refresh page; current activity indicator; admin (alert = break, accent = work, theme = inactivity/member activity)
                        <br/> <b>OldConsole Tabs</b>: today's scheduled roles, roles with available activities, controls oldConsoles
                        <br/> <b>Desk</b>: 
                    ...
                    <b>SOCIAL</b>: for users with social features enabled (Base + $2.00)
                        <br/> <b>Styling</b>: 
                        <br/> <b>Progress Bar</b>: 
                        <br/> <b>OldConsole Button</b>: opens sidebar, refresh page; alert medium = open request(s), th
                        <br/> <b>OldConsole Tabs</b>: only one selected; Voice, Collectives (Academy, Region, District, Community, Cluster, Connection Groups)
                        <br/> <b>Help Tool</b>: 
                        <br/> <b>Desk</b>: 
                    ...
                    <b>ASSESSMENT</b>: for users with assessment features enabled (Base + $2.00)
                        ...
                        <br/> <b>OldConsole Button</b>: opens sidebar, refresh page; opens and closes sidebar 
                        <br/> <b>OldConsole Tabs</b>: open assessments with % completion (only one open)
                        <br/> <b>Desk</b>: 
                    ...
                    <b>SCHOLAR</b>: for users with scholar features enabled (Base + $4.00)
                        ...
                        <br/> <b>OldConsole Button</b>: opens sidebar, refresh page; activity indicator (if breaktime setting is enabled, shows activity score)
                        <br/> <b>OldConsole Tabs</b>: General (active certifications, active courses, course catalog), Library, Course Tabs (lessons, projects, discussions, reading, assessment), Sandbox (practice, review, evaluation), Application (training, role, promotion)
                        <br/> <b>Desk</b>: 
                        <br/>
                        <hr></hr>
                    <b>ADMIN</b>: for admin, includes multiple roles ($10.00, includes scholar, assessment, social, and basic)
                        ...
                        <br/> <b>OldConsole Button</b>: opens sidebar, refresh page; activity indicator (alert bright = inactivity, accent bright = low activity, accent medium = average activity, accent dark = high activity), opens and closes sidebar 
                        <br/> <b>OldConsole Tabs</b>: only one selected; tab determines desk content; includes name of role and quantity of work  
                        <br/>
                        <br/>
                        <br/> <b>DESK</b>: 
                        <br/>
                    ...
                    <b>HELP DESK</b>: for admin;
                        ...
                        <br/> <b>OldConsole Button</b>: opens sidebar, refresh page; activity indicator (low activity is bright accent, medium activity is medium accent, high activity is dark accent)
                        <br/> <b>OldConsole Tabs</b>: chat heads (thumbnail, name, number of new replies, new reply is alert color, no new reply is accent color); open new chat
                        <br/>
                    To-Do List for OldConsole: 
                        ...
                        <br/>- save button on the oldConsole
                        <br/>- help desk toggle for admin oldConsoles
                        <br/> 
...
function HelpDeskChat() {
    const name = "Viola Davis";
    const helpToolState = "Open"; // Open Closed
    const userDetailState = "Closed";
    
    
    
    
    return(
    <section className={"helpDeskChat helpTool" + helpToolState}> {/* CONTROLS HELPTOOL OPEN/CLOSE */}
        <section className={"helpDeskChatContent userDetails" + userDetailState}> {/* CONTROLS USERDETAILS OPEN/CLOSE */}
            <header> {/* CONTROLS THE ACTIVE USERDETAILS TAB */}
                User Details
            </header>
            <section> 
                Chat Body
                {/* admin messages left, user messages right, chat actions center, response draft bubble bottom */}
            </section>
        </section>
        <section className='helpDeskChatTool'>  {/* CONTROLS THE ACTIVE HELPTOOL TAB */}
            <span></span>
            <nav>
                <button></button>
                <button></button>
            ...
            <section></section>
            {/* help tool title, help tool buttons (open/close helptool, link creator, tutorials, FAQs, quick replies, assisted feedback), help tool content (interfaces) */}
        </section>
        {/* 
            HelpChat To-Do:
            - Add height dependent styling for landscape phones for sidebar
            - 
            - 
            - 
            - 
